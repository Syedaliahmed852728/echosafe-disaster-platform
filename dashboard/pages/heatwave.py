"""Heatwave Risk Page - Prediction First with Map.

Behavior:
- Prediction summary is primary.
- Heatwave map is included.
...
class HeatwaveDataError(Exception):
    """Controlled heatwave page error."""
...
def signal_message(level: str) -> str:
    ...
    messages = {
        "None": "No heatwave expected today.",
        "Low": "Low heatwave signal today. Conditions are generally manageable, but basic precautions are still recommended.",
        "Medium": "Moderate heatwave signal today. Limit prolonged outdoor exposure and stay hydrated.",
        "High": "High heatwave signal today. Outdoor activity should be reduced, especially for vulnerable groups.",
        "Critical": "Critical heatwave signal today. Strong safety action is recommended and exposure should be minimized.",
...
def actual_heatwave_signal(
    ...
    """
    max_temp = safe_float(temperature_max_c)
    feels_like = safe_float(apparent_temperature_max_c)
    humidity = safe_float(humidity_percent)
    wind = safe_float(wind_speed_kmh)

    heat_stress = max(max_temp, feels_like)

    if heat_stress >= 47 or (max_temp >= 45 and humidity >= 35):
        return "Critical"

    if heat_stress >= 44 or max_temp >= 42 or (max_temp >= 40 and humidity >= 45):
        return "High"

    if heat_stress >= 40 or max_temp >= 38 or (max_temp >= 36 and humidity >= 55):
        return "Medium"

    if heat_stress >= 36 or max_temp >= 34 or (humidity >= 65 and wind <= 8):
        return "Low"

    return "None"


def inject_heatwave_css() -> None:
    h(
        """
        <style>
        ...
        .heat-hero {
            ...
            background:
                ...
                linear-gradient(135deg, #0f7a3a 0%, #159447 45%, #f97316 100%);
        ...
        .heat-hero::before {
            ...
            animation: heatGrid 18s linear infinite;
        ...
        .summary-card {
            background:
                radial-gradient(circle at 88% 9%, rgba(249,115,22,.10), transparent 30%),
        ...
        .prediction-card {
            background:
                radial-gradient(circle at 90% 5%, rgba(249,115,22,.16), transparent 32%),
...
async def _geocode_nominatim(city: str) -> dict | None:
    payload = await _get_json_async(
        ...
        headers={"User-Agent": "EchoSafe-HeatwaveDashboard/1.0"},
        timeout=15,
    )
...
async def _geocode_city_async(city: str) -> dict:
    ...
    raise HeatwaveDataError(f"No Pakistan city or location found for: {city}")
...
async def _fetch_weather_range_async(latitude: float, longitude: float, start: date, end: date) -> dict:
    ...
    if daily_df.empty:
        raise HeatwaveDataError("No weather records returned for the selected date range.")
...
async def _fetch_heatwave_bundle_async(city: str, start: date, end: date) -> dict:
...
def fetch_heatwave_bundle(city: str, start: date, end: date) -> dict:
    return _run_async(_fetch_heatwave_bundle_async(city, start, end))
...
def build_model_features(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not hourly.empty:
        ...
        daily["apparent_temperature_mean_c"] = None
    ...
    daily["apparent_temperature_mean_c"] = daily["apparent_temperature_mean_c"].fillna(
        daily["temperature_mean_c"]
    )

    if "apparent_temperature_max" not in daily.columns:
        daily["apparent_temperature_max"] = daily["apparent_temperature_mean_c"]

    daily["apparent_temperature_max"] = daily["apparent_temperature_max"].fillna(
        daily["apparent_temperature_mean_c"]
    ).fillna(daily["temperature_2m_max"])

    daily["month"] = pd.to_datetime(daily["date"].astype(str)).dt.month
    daily["quarter"] = ((daily["month"] - 1) // 3) + 1
    daily["season_enc"] = daily["month"].map(
        lambda month: 3 if month in [7, 8, 9] else 2 if month in [5, 6] else 0
    )
...
def predict_daily_heatwave(feature_df: pd.DataFrame, city: str) -> pd.DataFrame:
    rows = []

    for _, row in feature_df.iterrows():
        actual_signal = actual_heatwave_signal(
            temperature_max_c=safe_float(row.get("temperature_2m_max")),
            apparent_temperature_max_c=safe_float(row.get("apparent_temperature_max")),
            humidity_percent=safe_float(row.get("humidity_mean_percent")),
            wind_speed_kmh=safe_float(row.get("wind_speed_mean_kmh")),
        )
        ...
        input_data = {
            ...
            "month": int(safe_float(row.get("month"), 5)),
            "quarter": int(safe_float(row.get("quarter"), 2)),
            "season_enc": int(safe_float(row.get("season_enc"), 2)),
            "is_monsoon": int(safe_float(row.get("is_monsoon"), 0)),
        }

        try:
            model_output = predict_heatwave_risk(input_data)
        ...
        rows.append(
            {
                "date": row.get("date"),
                "condition": condition,
                "temperature_mean_c": input_data["temperature_mean_c"],
                "temperature_max_c": input_data["temperature_max_c"],
                "temperature_min_c": input_data["temperature_min_c"],
                "apparent_temperature_max_c": safe_float(row.get("apparent_temperature_max")),
...
def summarize_prediction(prediction_df: pd.DataFrame, city: str) -> dict:
    ...
    return {
        ...
        "temperature_max_c": safe_float(selected.get("temperature_max_c")),
        "apparent_temperature_max_c": safe_float(selected.get("apparent_temperature_max_c")),
        "humidity_percent": safe_float(selected.get("humidity_percent")),
        "wind_speed_kmh": safe_float(selected.get("wind_speed_kmh")),
...
def load_batch_heatwave_predictions() -> pd.DataFrame:
    ...
    if "disaster_type" in df.columns:
        df = df[df["disaster_type"].astype(str).str.contains("Heat", case=False, na=False)].copy()
...
def build_heatwave_map_points(
    selected_city: str,
    selected_lat: float,
    selected_lon: float,
    summary: dict,
) -> pd.DataFrame:
    batch_df = load_batch_heatwave_predictions()
    rows: list[dict] = []
            ...
            rows.append(
                {
                    ...
                    "message": row.get("message", "Heatwave prediction available."),
...
def render_heatwave_legend() -> None:
    h(
        """
...
def render_heatwave_map(summary: dict, location: dict) -> None:
    h(
        """
        <div class="section-title">Pakistan Heatwave Prediction Map</div>
        <p class="section-subtitle">Regional heatwave signals are shown on the map. The searched location is highlighted with its latest prediction.</p>
        """
    )

    render_heatwave_legend()

    map_df = build_heatwave_map_points(
...
def render_top_header() -> None:
        ...
        h(
            ...
            <div class="top-shell">
                <div class="top-brand">
                    ...
                    <div>
                        <div class="top-title">EchoSafe Heatwave Operations</div>
                        <div class="top-subtitle">City-level heatwave prediction and risk comparison</div>
                    </div>
                </div>
                <div class="top-subtitle">Heatwave Risk</div>
            </div>
            """
        )

    with top_right:
        if st.button("Logout", use_container_width=True, key="heatwave_logout"):
            logout()
            st.rerun()
    ...
    for col, (display_label, page_key) in zip(nav_cols, nav_items):
        with col:
            if st.button(display_label, use_container_width=True, key=f"heatwave_nav_{page_key}"):
                go_to_page(page_key)
    ...
    h(
        ...
        <div class="heat-hero">
            <div class="hero-inner">
                ...
                <h1 class="hero-title">Heatwave Risk Prediction</h1>
                <p class="hero-subtitle">
                    Search a city, choose a period, and compare observed heat conditions with the predicted heatwave risk signal.
                </p>
            </div>
        </div>
        """
...
def render_search_panel() -> None:
    h(
        ...
        <div class="search-panel">
            <div class="search-heading">Search Heatwave Prediction</div>
            <div class="search-subcopy">Select a location and date range to compare recent conditions with the expected heatwave risk.</div>
        </div>
        """
    )
    ...
    if "heatwave_city" not in st.session_state:
        st.session_state["heatwave_city"] = DEFAULT_CITY

    if "heatwave_start_date" not in st.session_state:
        st.session_state["heatwave_start_date"] = today - timedelta(days=6)

    if "heatwave_end_date" not in st.session_state:
        st.session_state["heatwave_end_date"] = today

    with st.form("heatwave_search_form", clear_on_submit=False):
        c1, c2, c3, c4 = st.columns([0.40, 0.19, 0.19, 0.17], gap="medium")

        with c1:
            city_input = st.text_input(
                "Location",
                value=st.session_state["heatwave_city"],
        ...
        with c2:
            start_input = st.date_input(
                "Start Date",
                value=st.session_state["heatwave_start_date"],
                min_value=min_date,
                max_value=max_date,
            )
        ...
        with c3:
            end_input = st.date_input(
                "End Date",
                value=st.session_state["heatwave_end_date"],
                min_value=min_date,
                max_value=max_date,
            )
    ...
    if submitted:
        ...
        st.session_state["heatwave_city"] = city_input.strip() or DEFAULT_CITY
        st.session_state["heatwave_start_date"] = start_input
        st.session_state["heatwave_end_date"] = end_input
        st.rerun()
...
def render_prediction_summary(summary: dict) -> None:
    ...
    mini_items = [
        ("Max Heat", f"{summary['temperature_max_c']:.1f}°C"),
        ("Feels Like", f"{summary['apparent_temperature_max_c']:.1f}°C"),
        ("Humidity", f"{summary['humidity_percent']:.0f}%"),
        ("Wind", f"{summary['wind_speed_kmh']:.1f} km/h"),
    ]
    ...
    h(
        ...
        <div class="summary-card">
            <div class="prediction-card">
                ...
                <h2 class="card-title">{esc(summary["city"])} Heatwave Risk</h2>
    ...
    for _, row in table.iterrows():
        ...
        rows.append(
            ...
            <tr>
                ...
                <td>{safe_float(row.get("temperature_max_c")):.1f}°C</td>
                <td>{safe_float(row.get("apparent_temperature_max_c")):.1f}°C</td>
                <td>{safe_float(row.get("humidity_percent")):.0f}%</td>
                <td>{safe_float(row.get("wind_speed_kmh")):.1f} km/h</td>
            ...
            <table class="records-table">
                <thead>
                    <tr>
                        ...
                        <th>Actual Max Heat</th>
                        <th>Actual Feels Like</th>
...
def render_heatwave_page() -> None:
    ...
    city = st.session_state.get("heatwave_city", DEFAULT_CITY)
    start_date = st.session_state.get("heatwave_start_date", local_today() - timedelta(days=6))
    end_date = st.session_state.get("heatwave_end_date", local_today())

    with st.spinner(f"Running heatwave prediction for {city}..."):
        try:
            bundle = fetch_heatwave_bundle(city, start_date, end_date)
            feature_df = weather_bundle_to_feature_frame(bundle, start_date, end_date)

            if feature_df.empty:
                raise HeatwaveDataError("No prediction data was generated for the selected date range.")

            prediction_df = predict_daily_heatwave(
            ...
            st.error("Heatwave prediction could not be completed. Please check the location, date range, model files, or internet connection.")
            st.caption(str(exc))
            return
    ...
    with top_right:
        render_heatwave_map(summary, bundle["location"])
