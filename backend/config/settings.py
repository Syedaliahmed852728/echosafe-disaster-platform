...
class PipelineSettings:
    ...
    # Rolling historical window every pipeline stage targets, in years. All
    # modules (earthquake, hailstorm, heatwave) reuse this so a single env var
    # (ECHOSAFE_LOOKBACK_YEARS) shifts the whole platform's training window.
    lookback_years: int = field(
        default_factory=lambda: _int_env("ECHOSAFE_LOOKBACK_YEARS", 10)
    )
