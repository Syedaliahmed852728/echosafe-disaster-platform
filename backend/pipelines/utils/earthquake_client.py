...
from backend.pipelines.utils.incremental import (
    covers_window,
    missing_windows,
    target_window,
)
...
def generate_fallback_earthquakes(count: int = 150) -> List[Dict]:
    ...
    years = SETTINGS.pipeline.lookback_years
    base_time = pd.Timestamp.now(tz="UTC") - pd.Timedelta(days=365 * years)
    events: List[Dict] = []
    for i in range(count):
        region = regions_df.iloc[i % len(regions_df)]
        days_offset = int(np.random.randint(0, 365 * years))
...
def fetch_earthquake_events(
    ...
    start_dt = (
        ...
        if start_time
        else today - timedelta(days=365 * SETTINGS.pipeline.lookback_years)
    )
    end_dt = (
...
def download_earthquake_data(output_path: Optional[Path] = None) -> None:
    ...
    target_start, today = target_window()
    ...
    if covers_window(existing, (target_start, today)):
        years = SETTINGS.pipeline.lookback_years
        logger.info(
            f"Bronze already covers the past {years} years "
            f"({existing[0].date()}..{existing[1].date()}); no scrape needed."
        )
        return

    windows = missing_windows(existing, (target_start, today))
    if not windows:
        return
