...
def _fragment_window(rows: List[Dict[str, Any]]) -> Optional[tuple]:
    """Return (min_dt, max_dt) UTC midnight from a fragment's rows."""
    if not rows:
        return None
    dates = []
    for r in rows:
...
def _merge_rows_by_date(
    ...
    """Merge per-day rows; newer rows for the same date override older ones."""
...
def download_heatwave_data(output_path: Optional[Path] = None) -> Path:
    """Pull Open-Meteo / NASA POWER daily history for every Pakistani region.

    Incremental: each region's fragment under
    ``data/bronze/heatwave/_fragments/<region>.json`` is reused. Only the
    head (if ``lookback_years`` was widened) and the tail (since the fragment
    was last updated) are fetched and merged.
    """
    ...
    for region in regions:
        ...
        if frag_path.exists():
            try:
                ...
                logger.warning(f"Fragment unreadable for {region['region']}: {exc}")

        existing_window = _fragment_window(cached_rows)
        if covers_window(existing_window, target):
            logger.info(
                f"-> {region['region']:14s} cached  ({len(cached_rows)} rows; "
                f"{existing_window[0].date()}..{existing_window[1].date()})"
            )
            daily_records.extend(cached_rows)
            continue

        gaps = missing_windows(existing_window, target)
        logger.info(
        ...
        for g_start, g_end in gaps:
            try:
                archive_rows = _fetch_archive_daily(region, g_start, g_end)
            except RuntimeError as exc:
                # NASA POWER may return zero rows for very short tail gaps that
                # fall inside its 2-3 day reanalysis lag. Fall through to the
                # Open-Meteo tail below.
                logger.info(
                    f"     NASA POWER empty for {g_start.date()}..{g_end.date()}: {exc}"
                )
                archive_rows = []
            if archive_rows:
                seg_last = max(r["date"] for r in archive_rows)
                logger.info(
                    f"     {len(archive_rows)} NASA POWER rows for "
                    f"{g_start.date()}..{g_end.date()} (latest {seg_last})"
                )
                if last_archive_date is None or seg_last > last_archive_date:
                    last_archive_date = seg_last
                new_rows.extend(archive_rows)

        # NASA POWER lags by ~2-3 days; bridge with Open-Meteo for whatever's
        # missing after the latest archive row (or after the existing cache
        # if no new archive rows landed).
        anchor = last_archive_date
        if anchor is None and existing_window is not None:
            anchor = existing_window[1].strftime("%Y-%m-%d")
        if anchor is None:
            anchor = start.strftime("%Y-%m-%d")
        tail = _fetch_open_meteo_tail(
            region, after_date=anchor, today_date=today_date
        )
        if tail:
            logger.info(
            ...
            new_rows.extend(tail)

        merged = _merge_rows_by_date(cached_rows, new_rows)
        merged_window = _fragment_window(merged)
        with open(frag_path, "w") as fh:
            json.dump(
                {
                    "region": region["region"],
                    "start": (merged_window[0].strftime("%Y-%m-%d")
                              if merged_window else start.strftime("%Y-%m-%d")),
                    "end": (merged_window[1].strftime("%Y-%m-%d")
                            if merged_window else end.strftime("%Y-%m-%d")),
                    "rows": merged,
                },
                fh,
                default=str,
            )
        daily_records.extend(merged)
        if new_rows:
            time.sleep(1)  # courtesy delay only when we actually hit the API

    payload = {
        "metadata": {
            "source": "NASA POWER daily archive (+ Open-Meteo tail)",
